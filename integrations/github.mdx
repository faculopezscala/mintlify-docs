...
- If no URL is provided, tests use the `base_url` from `.bugster/config.json`

## Configuration

Ensure your `.bugster/config.json` has the correct base URL as fallback:
...
<AccordionGroup>
  ...
  <Accordion icon="settings" title="Missing environment URL">
    **Result:**
    Tests will run against the `base_url` in your `.bugster/config.json` instead of the deployment URL.
  </Accordion>
</AccordionGroup>
