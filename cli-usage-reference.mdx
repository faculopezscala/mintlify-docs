...
The Bugster CLI provides a comprehensive set of commands for authentication, project setup, test generation, execution, and synchronization. Here are all available commands:
...
<Card title="Authenticate with Bugster" icon="key">
  Connect your CLI to your Bugster account by setting up your API key. This is the first command you should run.
</Card>
...
<Tip>
  You must authenticate before using other Bugster CLI features that require API access.
</Tip>
...
- Loads configuration from `.bugster/config.json`
...
<AccordionGroup>
  <Accordion icon="rocket" title="Getting Started">
    ```bash
    # 1. Authenticate with your API key (FIRST STEP)
    bugster auth
    
    # 2. Initialize project
    bugster init
    
    # 3. Generate initial test specs
    bugster generate
...
<AccordionGroup>
  <Accordion icon="exclamation-triangle" title="Common Issues">
    **Authentication Problems:**
    - Verify your API key is correct: `bugster auth`
    - Check network connectivity to Bugster servers

    **Configuration Issues:**
    - Check `.bugster/config.json` for syntax errors
  ...
  <Accordion icon="refresh" title="Reset and Recovery">
    ...
    # Reinitialize project (preserves tests)
    bugster init
